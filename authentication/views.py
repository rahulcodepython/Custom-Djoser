...
class UserViews(views.APIView):
    ...
    def check_email_exists(self, email):
        return True if User.objects.filter(
            email=email).exists() else False
    ...
    def get(self, request):
        try:
            if self.check_authenticated_user(request.user) is False:
                res = Message.error(
                    "You are not authenticated yet. Try again.")
    ...
    def post(self, request):
        try:
            ...
            res = Message.create(
                "Your account has been creates. At First verify it.")
            return response.Response({**res["body"], **{'user': user.email}}, status=res['status'])
...
class ActivateUserViews(views.APIView):
    def get(self, request):
        try:
            ...
            user = models.ActivationCode.objects.filter(uid=uid, token=token)[
                0].user if models.ActivationCode.objects.filter(uid=uid).filter(token=token).exists() else None

            if user is None:
                res = Message.error(
                    "No such user is there. Try again.")
                return response.Response(res['body'], status=res['status'])
            ...
            return response.Response({**res['body'], **get_tokens_for_user(parent_user)}, status=res['status'])
...
class CreateJWT(views.APIView):
    def post(self, request):
        try:
            ...
            if user is None:
                res = Message.error(
                    "You are not authenticated properly. Try again.")
                return response.Response(res['body'], status=res['status'])

            if user.is_active is False:
                res = Message.error(
                    "You are not verified yet. Verify first.")
                return response.Response(res['body'], status=res['status'])

            res = Message.success(
                "You are authenticated.")
            return response.Response({**res['body'], **get_tokens_for_user(user)}, status=res['status'])
...
class ResetUserPassword(views.APIView):
    ...
    def post(self, request):
        try:
            ...
            if models.ResetPasswordCode.objects.filter(uid=uid, token=token).exists() is False:
                res = Message.error(
                    "There is some issue. Try again.")
                return response.Response(res['body'], status=res['status'])

            current_user = models.ResetPasswordCode.objects.filter(uid=uid, token=token)[
                0]
